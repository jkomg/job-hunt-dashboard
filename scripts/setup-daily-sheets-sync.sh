...
JOB_NAME="${JOB_NAME:-job-hunt-daily-sheets-sync}"
...
if [[ -z "${CRON_TOKEN}" ]]; then
  echo "Missing CRON_TOKEN. Provide it from SHEETS_SYNC_CRON_TOKEN."
  exit 1
fi
...
URI="${SERVICE_URL}/api/internal/sheets/sync"
if [[ -n "$DOMAIN" ]]; then
  URI="https://${DOMAIN}/api/internal/sheets/sync"
...
HEADERS="x-sync-token=${CRON_TOKEN}"
